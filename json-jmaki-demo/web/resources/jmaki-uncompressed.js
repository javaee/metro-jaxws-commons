...
function Jmaki() {
        ...
        if (_jmaki.config  &&
            ...
            for (var _l=0; _l < _jmaki.config.glue.listeners.length;_l++ ) {
                var _listener = _jmaki.config.glue.listeners[_l];
                    ...
                    (typeof _listener.topic.charAt == 'function' && _listener.topic.charAt(_listener.topic.length-1)== '*'  && (name.indexOf(_listener.topic.substring(0,_listener.topic.length -1)) == 0)  &&
                       name.substring(0, _listener.topic.length-1) == _listener.topic.substring(0,_listener.topic.length-1) ) ||
                    (typeof _listener.topic.charAt == 'function' &&_listener.topic.charAt(0) == '*'   && name.indexOf(_listener.topic.substring(1,_listener.topic.length) != -1) &&
                    name.substring(name.indexOf(_listener.topic.substring(1,_listener.topic.length)) , name.length) == _listener.topic.substring(1,_listener.topic.length) ) ||
                     (typeof _listener.topic.charAt == 'function' && _listener.topic.charAt(0) == '*'  &&_listener.topic.charAt(_listener.topic.length-1)== '*'  &&
    ...
    this.addLibraries = function(_libs, _cb, _inprocess) {
        ...
        if (_jmaki.blocked) {
            ...
            for (var b=0; b < _jmaki.blocked.length; b++) {
                if (_lib.indexOf(_jmaki.blocked[b]) != -1) {
                    return _lib;
                }
            }
        }
    ...
    */
    this.doAjax= function(args) {
       ...
       if (args.method) {
            method=args.method;
            if (method.toLowerCase() == 'post' && !args.headers) {
                _req.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
            }
       }
       if (args.headers) {
           for (var _h=0; h < args.headers.length; _h++) {
   ...
   /**
    ...
    this.loadScript = function(target) {
        ...
        try {
           ...
           _jmaki.log("error loading : " + target);
        }
        if (req.status == 200) {
  ...
  function getContent(rawContent, _task) {
   ...
   _task.styleReferences = [];
    ...
    this.addLibraries = function(_libs, _cb, _inprocess) {
        ...
        var _lib = _libs[_libs.length-1];
        return _jmaki.addLibraries(_libs,_cb,_inprocess);
    }
  }
